...
export const Navbar: React.FC<NavbarProps> = ({ currentTab, setCurrentTab, openAIChat }) => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full overflow-x-clip bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 transition-colors">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-16 flex items-center gap-2 sm:gap-4">
        ...
        >
          <PltLogo size="md" />
          <span className="hidden sm:inline font-black text-base lg:text-lg tracking-tight bg-linear-to-r from-indigo-600 to-blue-600 dark:from-indigo-400 dark:to-blue-400 bg-clip-text text-transparent whitespace-nowrap">
            STUDENT STUDY
          </span>
        </div>

        {/* Desktop Navigation */}
        <nav className="hidden md:flex flex-1 items-center justify-end gap-1 lg:gap-1.5 min-w-0">
          {navItems.map(item => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-3.5 lg:px-4 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all ${
                  isActive
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          ...
          {currentUser ? (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 p-1 pl-2 min-h-10 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
              >
          ...
          <button
            ...
            className="md:hidden p-2.5 min-w-11 min-h-11 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"
          >
          ...
          {navItems.map(item => {
            ...
            return (
              <button
                ...
                className={`w-full min-h-11 flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-semibold transition-all ${
                  isActive
